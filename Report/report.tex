...
Our descriptor is then a vector $\vec{d}(I)$, of $n$ values, where each value is simple the average of frequency values in specific bin, that is then normalized by $N = \sum_x,y A(x,y)$, the average of all values in an image. This way, each element $d_i$ gives us a percentage contribution from each frequency range. We will discuss the performance in chapter \ref{chap:analysis}.\\
...
To capture local noise in singular vectors we decide to take the FFT of the singular vectors rather than use them directly. This will capture the high frequency components of a noisy vector (such as the forest in \ref{fig:natural_vectors}) as well as the low frequency changes of a coastline or a street.
...
In the first row we observe a clear improvement especially in terms of three nearest neighbors, all showing a view down the street. Clearly computing the localized version of a descriptor has improved the query results. Other two neighbors are still city scenes with similar color distribution.

The second row is still a hard case for our descriptor, however now $3$ out of $5$ neighbors are also in town images, and interestingly all neighbors are scenes showing the outdoor environment during the sunset. We argue that localized version of $L^*a^*b^*$ color descriptor are picking up the perceived lightness correctly. Again as in the case of non-windowed versions all neighbors exhibit similar frequency distribution with strong low frequency components and some additional strong contributions form higher frequencies.

For the third row, we observe the decline in performance, the last neighbor is not a mountain anymore. This is caused by the fact that mountains have very descriptive global structure, that might not be well preserved in local patches. Still the last neighbor has very similar color, and also have quite complex texture on building's facade.

Fourth row is also a clear improvement where all neighbors are showing similar scenes of deserts. We still pick up the image with the road as our third nearest neighbor, however all images are perceptually similar. The factor that seems to be grouping them together is the strong horizontal periodicity in bottom half of all images, that is successfully picked up by our frequency descriptor. 

Fifth row is also improvement, where all neighbors have similar orientations, showing us mostly images of coast lines, with the exception of neighbor $3$. The reason it is showing is the strong contribution from the high frequencies (grass), that is also present in query image (leaves in the foreground ). Still the color distribution is close by, with grass of neighbor $3$ of dark brown color, that is a close in $L^*a^*b^*$ to the light brown of the coastline sand.

Overall the windowed version of our descriptor is performing better then non windowed version, across different categories. The localization of descriptors allows us to capture finer details that may have otherwise been lost during the processing of a full image. This comes at the cost of increased computation and descriptor length. A cost-performance analysis could be done on both the grid size and the weighting of each descriptor, but we feel that is beyond the scope of this study.

We also provide a results of $k$-means clustering with $k=6$ in Appendix \ref{sec:appendix}. The $k$-means clustering shows that our aggregate descriptor is able to clearly differentiate images be overall structure. The top cluster shows images that are planes (e.g. roads, beaches, horizons), the second cluster shows images with more local structure (e.g. forests), the forth cluster shows images of large structures (e.g. mountains) and the sixth cluster shows a grouping of man made objects (e.g. building facades, streets, and alleyways). The third and fifth clusters are less clearly defined, but images in the third cluster seem to share common (somewhat unique) colors and images in the fifth cluster seem to share the general structure of a simple ground level and a sky.

The MDS visualization of our space in Appendix \ref{sec:appendix} shows that the aggregate descriptor performs very well at spreading images out based on different properties. On the horizontal axis, images are differentiated by both color and overall simplicity. Simple horizons are on the far right whereas forests and dense images are on the far left. Lighter images are higher on the vertical axis than darker images. Within the space a clear clustering of image types is visible; most mountains are near other mountains, most city streets are near other city streets, etc.

\chapter{Conclusions and Further Work}
\label{chap:Conclusions}
Work done during this project was of an exploratory nature, where we have investigated what cues for image understanding can be obtained by analyzing the outputs of FFT and SVD algorithms. We have found that these methods do provide us with useful information when analyzing natural images, and can be easily used in generating meaningful low level descriptors, as shown in previous chapter. Fourier domain informs us about the overall image texture and roughness. Similarly our analysis of Singular Value Decomposition shows that the singular vectors can tell us about the image geometry, and could be used for matching images of similar orientation. Our results show an embedding that successfully clusters images of similar nature quite well, however the algorithm may not be  competitive in terms of performance with state-of-the-art algorithms.
