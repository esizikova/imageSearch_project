...
for i = 1:num_sv
    ...
    subplot(num_rows,num_sv+1,i+1); imshow(sv_img); title(['SV',int2str(i),': ', int2str(S(i,i))])
    subplot(num_rows,num_sv+1,i+num_sv+2); plot(U(:,i)); xlim([1,256]); title('U'); set(gca, 'XTickLabel', [])
    subplot(num_rows,num_sv+1,i+(num_sv+1)*2+1); plot(V(:,i)); xlim([1,256]); title('V'); set(gca, 'XTickLabel', [])
